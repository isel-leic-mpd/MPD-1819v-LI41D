...
public class WeatherWebApiCsv extends BaseWeatherApi {
    ...
    public WeatherInfo[] pastWeather(double lat, double log, LocalDate from, LocalDate to) throws IOException {
        ...
        int[] i = {0};
//        return request.getLines(getPastWeatherUrl(lat, log, from, to))
...
//                .map(WeatherInfo::valueOf);
     ...
     */
    @Override
    public LocationInfo[] search(String query) throws IOException {
        if(query == null || query.isEmpty()) {
            throw new IllegalArgumentException("query cannot be null or empty");
        }

//        return request.getLines(getSearchUrl(query))
//                .filter(WeatherWebApiCsv::isNotComment)
